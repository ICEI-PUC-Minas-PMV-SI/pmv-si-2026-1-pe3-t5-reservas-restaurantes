  ...
  return `
    <div class="client-navbar-actions client-navbar-actions-full">
      <button id="calendarReservationsBtn" class="notification-btn" type="button">
        <span class="nav-text">${isLoggedAdmin ? "Painel admin" : "Minhas reservas"}</span>
      </button>
      ...
      >
        ...
        <span id="notificationCount" class="notification-count-badge">0</span>
      </button>
      ...
      <div class="profile-menu-wrapper">
        ...
        <div id="profileDropdown" class="profile-dropdown">
          ...
          ${
            isLoggedAdmin
              ? `<a href="../pages/admin.html">Painel administrativo</a>`
              : `<a href="../pages/client.html#avaliacoes">Minhas avaliações</a>`
          }
    ...
    <main class="restaurant-detail-page">
      <header class="client-public-topbar">
        <div class="details-navbar-container">
          ...
          ${isLoggedUser ? renderLoggedHeader() : renderPublicHeader()}
        </div>
      </header>
      ...
      <section class="detail-layout">
        ...
        <aside class="reservation-sidebar">
          ...
          <form id="detailReservationForm">
            ...
            <label class="detail-table-field" for="detailSelectedTable">
              Mesa
            </label>

            <select id="detailSelectedTable" required>
              <option value="">Selecione data, horário e pessoas</option>
            </select>

            <button type="submit" class="btn" style="width:100%; margin-top:20px;">
              ${isLoggedUser ? "Concluir reserva" : "Entrar para reservar"}
            </button>
          </form>
        </aside>
      </section>
...
function bindEvents() {
  document.addEventListener("click", (event) => {
    ...
    if (event.target.id === "calendarReservationsBtn" || event.target.closest("#calendarReservationsBtn")) {
      window.location.href = isLoggedAdmin ? "./admin.html" : "./client.html#reservas";
    }
...
function createReservationFromDetails() {
  ...
  const tableId = Number(document.getElementById("detailSelectedTable").value);
  ...
  if (!date || !people || !time || !tableId) {
    showToast("Selecione data, pessoas, horário e mesa.", "danger");
    return;
  }
  ...
  if (!table) {
    showToast("Selecione uma mesa disponível para esse horário.", "danger");
    return;
  }
...
/**
 * Finds the selected table when it is available for the requested slot.
 *
 ...
 * @returns {Object|undefined} Selected table when available.
 */
...
/**
 * Gets every table that fits the group and is free at the selected time.
 *
 * @param {Object} restaurant - Restaurant data.
 * @param {string} date - Reservation date.
 * @param {string} time - Reservation time.
...
/**
 * Renders table choices for the selected date, time, and party size.
 *
 * @returns {void}
 */
function renderDetailTableOptions() {
  const tableSelect = document.getElementById("detailSelectedTable");

  if (!tableSelect) return;

  const date = document.getElementById("detailDate")?.value || "";
  const time = document.getElementById("detailSelectedTime")?.value || "";
  const people = Number(document.getElementById("detailPeople")?.value || 0);
  const tables = getAvailableTables(restaurant, date, time, people);

  if (!date || !time || !people) {
    tableSelect.innerHTML = `<option value="">Selecione data, horário e pessoas</option>`;
    return;
  }
