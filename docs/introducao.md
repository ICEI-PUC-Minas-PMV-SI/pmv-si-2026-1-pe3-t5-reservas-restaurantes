...
O interesse crescente por tecnologias no setor alimentício reforça sua importância estratégica. A adesão a essas ferramentas depende, fundamentalmente, da percepção de utilidade e da facilidade de uso por parte do público. Portanto, implementar um sistema de reservas não representa apenas uma inovação técnica, mas uma estratégia de reorganização operacional capaz de elevar tanto a eficiência do serviço quanto a satisfação do cliente.

## 1.1. Problema

Apesar do avanço tecnológico observado no setor de hospitalidade, muitos restaurantes de pequeno e médio porte ainda realizam o controle de reservas de maneira manual ou por meio de ferramentas não integradas, como planilhas eletrônicas e aplicativos de mensagens instantâneas.
A literatura aponta que a ausência de sistemas estruturados pode resultar em falhas operacionais, conflitos de agendamento, sobreposição de reservas e dificuldades no controle da capacidade instalada (SUTJIADI et al., 2024). Além disso, a falta de automação limita a coleta e análise de dados estratégicos, reduzindo a capacidade do gestor de identificar padrões de demanda e otimizar a ocupação das mesas.
Embora pesquisas demonstrem que a automação de serviços contribui para maior eficiência e organização operacional (MAIER; EDWARDS, 2020), ainda existem desafios relacionados à implementação de soluções tecnológicas acessíveis e adequadas à realidade de pequenos estabelecimentos.
Assim, o problema central que orienta este trabalho consiste em compreender como a organização digital do processo de reservas pode contribuir para maior controle operacional, redução de conflitos de agendamento e melhoria da experiência do usuário em restaurantes de pequeno e médio porte.
